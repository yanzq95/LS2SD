...
class DepthCompletionNet(nn.Module):
    def __init__(self, args):
        ...
        self.DC_4x_to_1x = convt_bn_relu(64, 64, 4, 4, 0, 0)
        self.SR_1x_to_4x = conv_bn_relu(64, 64, 4, 4, 0)
        ...
        self.dc_and_sr_field_transform_3 = field_transform(128, 32, 152)
        self.dc_and_sr_field_transform_2 = field_transform(64, 64, 304)
        self.dc_and_sr_field_transform_1 = field_transform(64, 64, 304)
    ...
    def forward(self, z):
        ...
        sparse = F.max_pool2d(sparse, 4, 4) # B, 1, 64, 304
        # rgb = F.max_pool2d(rgb, 4, 4)

        # sparse encoding ############################################################
        conv1_d = self.conv1_d(sparse) # B, 16, 64, 304
        ...
        conv2_img = self.conv2_img2(conv2_img) # B, 64, 128, 608

        conv3_img = self.conv3_img(conv2_img) # 4x
        conv3_img = self.conv3_img2(conv3_img) # B, 48, 64, 304


        # encoder net ################################################################
        conv1 = torch.cat((conv1_d, conv3_img), 1)  # B, 64, 64, 304
        conv2 = self.conv2(conv1)  # B, 64, 64, 304
        conv3 = self.conv3(conv2)  # B, 128, 32, 152
        conv4 = self.conv4(conv3)  # B, 256, 16, 76
        conv5 = self.conv5(conv4)  # B, 512, 8, 38



        # DC and SR decoder ##################################################################
        # 4
        convt4 = self.convt4(conv5)  # B, 256, 16, 76
        y = convt4 + 0.5 * conv4

        sr_convt4 = self.srconvt4(conv5)  # B, 256, 16, 76
        ...
        # 3
        convt3 = self.convt3(y)  # B, 128, 32, 152
        y = convt3 + 0.5 * conv3

        sr_convt3 = self.srconvt3(x)  # B, 128, 32, 152
        ...
        # 2
        convt2 = self.convt2(y)  # B, 64, 64, 304
        y = convt2 + 0.5 * conv2

        sr_convt2 = self.srconvt2(x)  # B, 64, 64, 304
        ...
        # 1
        convt1 = self.convt1(y)   # B, 64, 64, 304
        y = convt1 + 0.5 * conv1
        ...
        DC_4x = self.DC(y)  # B, 64, 64, 304

        DC_1x = self.DC_4x_to_1x(DC_4x)   # B, 64, 256, 1216

        x = torch.cat((x, conv3_img), 1)
        x = self.srconvt2x(x)  # B, 64, 128, 608

        x = torch.cat((x, conv2_img), 1)
        x = self.srconvt4x(x)  # B, 64, 256, 1216

        x = torch.cat((x, conv1_img), 1)

        x = self.SR(x) # B, 64, 256, 1216

        SR_4x = self.SR_1x_to_4x(x)

        SR_cat_DC = torch.cat((x, DC_1x), 1)
        DC_cat_SR = torch.cat((DC_4x, SR_4x), 1)
