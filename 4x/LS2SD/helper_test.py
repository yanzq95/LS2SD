...
import torch
import csv
import vis_utils_test as vis_utils
...
class logger:
    ...
    def conditional_save_pred(self, mode, i, pred, Rmse):
        # if ("test" in mode or mode == "eval") and self.args.save_pred:
        if "test" in mode or mode == "eval" or mode=="val":
            ...
            filename_vis = os.path.join(image_folder, '{0:010d}'.format(i) + '_vis.png')
            vis_utils.save_depth_as_uint16png(img, filename)
            vis_utils.save_depth_as_uint16png_vis(img, filename_vis)

            # f = open('val_result.txt', 'a')
            # f.write('%d %.4f\n' % (i, Rmse*1000))
            # # print (format_str % (step, error['RMSE']), file=fl)
            # f.close()
