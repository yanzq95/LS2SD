...
data_path = os.getcwd().replace('2x/LS2SD', '') + 'data/'
...
def iterate(mode, args, loader, model, optimizer, logger, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        start = time.time()
        dc, pred = model(batch_data)
        depth_loss, photometric_loss, smooth_loss, mask = 0, 0, 0, None
        Rmse = depth_criterion(pred, gt)
        if mode == 'train':
            ...
            if 'sparse' in args.train_mode:
                ...
                # sim_loss = loss_sim(y1, y2)
                # print('sim_loss is :', sim_loss)
                # depth_loss = depth_loss + sim_loss
                # print('total_loss is :', depth_loss)
                # mask = (gt < 1e-3).float()
        ...
        with torch.no_grad():
            ...
            logger.conditional_save_img_comparison(mode, i, batch_data, pred,
                                                   epoch)
            logger.conditional_save_pred(mode, i, pred, Rmse)#######################################################################
...
def main():
    ...
    if is_eval:
        ...
        result, is_best = iterate("val", args, val_loader, model, None, logger,
                                  checkpoint['epoch'])
        ##################################################################################################################################
        # result, is_best = iterate("test_completion", args, test_loader, model, None, logger,
        #                           checkpoint['epoch'])
        return
